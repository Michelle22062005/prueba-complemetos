...
async function getMessages(req, res) {
    try {
        ...
        const cursorCreatedAt = typeof req.query.cursor_created_at ===
            "string"
            ? req.query.cursor_created_at
            : null;
        const cursorMessageId = typeof req.query.cursor_message_id ===
            "string"
        ...
        if (cursorMessageId !== null &&
            ...
            return res.status(400).json({
                error: "cursor_message_id inválido",
            });
        }
        ...
        const result = await (0, postgres_1.queryAsUser)(userId, async (client) => {
            return client.query(`
            SELECT
              m.rw_message_id,
              m.rw_channel_id,
        ...
        if (rows.length > 0) {
            ...
            nextCursor = {
                created_at: lastMessage.rw_created_at,
                message_id: lastMessage.rw_message_id,
            };
        }
...
async function createMessage(req, res) {
    try {
        ...
        if (typeof channel_id !== "string" ||
            ...
            return res.status(400).json({
                error: "channel_id es requerido y debe ser un UUID válido",
            });
        }
